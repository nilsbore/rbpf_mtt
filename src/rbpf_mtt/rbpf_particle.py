    ...
    def sample_update(self, nbr_observations, likelihoods):
        ...
        while True:
            ...
            for k in range(0, nbr_targets):
                ...
                states[k] = np.random.choice(2*nbr_observations+1, p=likelihoods[k])
                if states[k] < 2*nbr_observations:
                    sampled_states[k] = states[k] % nbr_observations
                else:
                    no_meas += 1

            unique, counts = np.unique(states, return_counts=True)

            if len(unique) + no_meas == nbr_targets:
                unique_list = unique.tolist()
                for j in unique_list:
                    self.c.append(j)
                for j in range(0, no_meas):
                    self.c.append(-1) # TODO: This is just a hack to please the smoother saving
                break

            nbr_jumps = np.sum(states >= nbr_observations) - no_meas
            nbr_assoc = np.sum(states < nbr_observations)
            print "Found assoc: ", nbr_assoc, ", nbr jumps: ", nbr_jumps, ", no meas: ", no_meas
            print "Continuing...."
    ...
    def joint_update(self, spatial_measurements, feature_measurements, time, observation_id):
        ...
        for k, i in enumerate(states):
            ...
            if i == 2*nbr_observations:
                ...
                weights_update *= pnone #1.0#likelihoods[k, i]/pc[k, i]
